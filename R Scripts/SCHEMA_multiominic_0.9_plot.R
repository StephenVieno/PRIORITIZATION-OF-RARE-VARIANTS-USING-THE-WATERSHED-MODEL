...
# Variants prioritized by the watershed model at PP > 0.9 in case genes
Schema_GRCh38_Watershed_0.9_case <- Schema_GRCh38_watershed_case |> 
  dplyr::filter(RNA > 0.9 | Methylation > 0.9 | Splicing > 0.9| Protein > 0.9) |> 
  mutate(Prioritized = ifelse(Watershed == "Yes", "Watershed_0.9", "No")) |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at PP > 0.9 in case genes
Schema_GRCh38_Watershed_0.9_control <- Schema_GRCh38_watershed_control |> 
  dplyr::filter(RNA > 0.9 | Methylation > 0.9 | Splicing > 0.9| Protein > 0.9) |> 
  mutate(Prioritized = ifelse(Watershed == "Yes", "Watershed_0.9", "No")) |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at RNA PP > 0.9 in case genes
Schema_GRCh38_RNA_0.9_case <- Schema_GRCh38_watershed_case |> 
  mutate(Prioritized = ifelse(RNA > 0.9, "RNA_0.9", "No")) |> 
  dplyr::filter(Prioritized == "RNA_0.9") |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at Methylation_0.9 PP > 0.9 in case genes
Schema_GRCh38_Methyl_0.9_case <- Schema_GRCh38_watershed_case |> 
  mutate(Prioritized = ifelse(Methylation > 0.9, "Methylation_0.9", "No")) |> 
  dplyr::filter(Prioritized == "Methylation_0.9") |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at Splicing PP > 0.9 in case genes
Schema_GRCh38_Splicing_0.9_case <- Schema_GRCh38_watershed_case |> 
  mutate(Prioritized = ifelse(Splicing > 0.9, "Splicing_0.9", "No")) |> 
  dplyr::filter(Prioritized == "Splicing_0.9") |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at Protein PP > 0.9 in case genes
Schema_GRCh38_Protein_0.9_case <- Schema_GRCh38_watershed_case |> 
  mutate(Prioritized = ifelse(Protein > 0.9, "Protein_0.9", "No")) |> 
  dplyr::filter(Prioritized == "Protein_0.9") |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at RNA PP > 0.9 in control genes
Schema_GRCh38_RNA_0.9_control <- Schema_GRCh38_watershed_control |> 
  mutate(Prioritized = ifelse(RNA > 0.9, "RNA_0.9", "No")) |> 
  dplyr::filter(Prioritized == "RNA_0.9") |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at Methylation PP > 0.9 in control genes
Schema_GRCh38_Methyl_0.9_control <- Schema_GRCh38_watershed_control |> 
  mutate(Prioritized = ifelse(Methylation > 0.9, "Methylation_0.9", "No")) |> 
  dplyr::filter(Prioritized == "Methylation_0.9") |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at Splicing PP > 0.9 in control genes
Schema_GRCh38_Splicing_0.9_control <- Schema_GRCh38_watershed_control |> 
  mutate(Prioritized = ifelse(Splicing > 0.9, "Splicing_0.9", "No")) |> 
  dplyr::filter(Prioritized == "Splicing_0.9") |> 
  dplyr::filter(est != "NA") 

# Variants prioritized by the watershed model at Protein PP > 0.9 in control genes
Schema_GRCh38_Protein_0.9_control <- Schema_GRCh38_watershed_control |> 
  mutate(Prioritized = ifelse(Protein > 0.9, "Protein_0.9", "No")) |> 
  dplyr::filter(Prioritized == "Protein_0.9") |> 
  dplyr::filter(est != "NA") 
...
Prioritized_data_0.9 <- bind_rows(Schema_GRCh38_Watershed_0.9_case, 
                                  Schema_GRCh38_Watershed_0.9_control,
                                  Schema_GRCh38_RNA_0.9_case, 
                                  Schema_GRCh38_Methyl_0.9_case, 
                                  Schema_GRCh38_Splicing_0.9_case, 
                                  Schema_GRCh38_Protein_0.9_case,
                                  Schema_GRCh38_RNA_0.9_control, 
                                  Schema_GRCh38_Methyl_0.9_control, 
                                  Schema_GRCh38_Splicing_0.9_control, 
                                  Schema_GRCh38_Protein_0.9_control)

# Create order for plotting 
order <-c("watershed-control&Methylation_0.9", "watershed-case&Methylation_0.9", 
          "watershed-control&Protein_0.9", "watershed-case&Protein_0.9",
          "watershed-control&RNA_0.9", "watershed-case&RNA_0.9", 
          "watershed-control&Splicing_0.9", "watershed-case&Splicing_0.9", 
          "watershed-control&Watershed_0.9", "watershed-case&Watershed_0.9")

# Create data frame for plotting 
plot_data <- Prioritized_data_0.9 |> 
...
my_comparisons <- 
  list(c("watershed-control&Methylation_0.9", "watershed-case&Methylation_0.9"), 
       c("watershed-control&Protein_0.9", "watershed-case&Protein_0.9"),
       c("watershed-control&RNA_0.9", "watershed-case&RNA_0.9"), 
       c("watershed-control&Splicing_0.9", "watershed-case&Splicing_0.9"), 
       c("watershed-control&Watershed_0.9", "watershed-case&Watershed_0.9"))

# Count of variants for each status definition 
count1 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-control&Methylation_0.9") |> 
  count()

count2 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-case&Methylation_0.9") |> 
  count()

count3 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-control&Protein_0.9") |> 
  count()

count4 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-case&Protein_0.9") |> 
  count()

count5 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-control&RNA_0.9") |> 
  count()

count6 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-case&RNA_0.9") |> 
  count()

count7 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-control&Splicing_0.9") |> 
  count()

count8 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-case&Splicing_0.9") |> 
  count()

count9 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-control&Watershed_0.9") |> 
  count()

count10 <- plot_data |> 
  dplyr::filter(status_Prioritized == "watershed-case&Watershed_0.9") |> 
  count()

# Schema_variants_0.9_plot
Schema_variants_0.9_plot <- plot_data |> 
  mutate(status_Prioritized = paste0(status,"&",Prioritized)) |> 
  mutate(status_Prioritized = factor(status_Prioritized, levels = order)) |> 
  ggplot(aes(y = rank_est, 
             x = status_Prioritized)) + 
  geom_boxplot(aes( 
    fill = status, 
    alpha = 0.5)) + 
  theme_minimal() + 
  ...
  geom_signif(
    ...
    textsize = 3, 
    test = "wilcox.test") + 
  scale_x_discrete(labels = c("watershed-control&Methylation_0.9" = paste0("Control genes
& Methylation > 0.9
(N = ", count1,")"), 
                              "watershed-case&Methylation_0.9" = paste0("Case genes
& Methylation > 0.9
(N = ", count2,")"),
                              "watershed-control&Protein_0.9" = paste0("Control genes
& Protein > 0.9
(N = ", count3,")"),
                              "watershed-case&Protein_0.9" = paste0("Case genes
& Protein > 0.9
(N = ", count4,")"),
                              "watershed-control&RNA_0.9" = paste0("Control genes
& RNA > 0.9
(N = ", count5,")"),
                              "watershed-case&RNA_0.9" = paste0("Case genes
& RNA > 0.9
(N = ", count6,")"),
                              "watershed-control&Splicing_0.9" = paste0("Control genes
& Splicing > 0.9
(N = ", count7,")"),
                              "watershed-case&Splicing_0.9" = paste0("Case genes
& Splicing > 0.9
(N = ", count8,")"),
                              "watershed-control&Watershed_0.9" = paste0("Control genes
& Watershed > 0.9
(N = ", count9,")"),
                              "watershed-case&Watershed_0.9" = paste0("Case genes
& Watershed > 0.9
(N = ", count10,")")))
