...
# Number of watershed variants 
unique(uv_watershed$RV) |> length()
# 19233

# Number of functional annotaitons 
unique(uv_watershed$RV) |> length()
# 19589
...
Watershed <- Schema_GRCh38 |> 
  dplyr::filter(Watershed == "Yes")

# Number of final watershed variants 
dim(Watershed)

# Number of final functional annotaitons 
unique(Watershed$RV) |> length()
